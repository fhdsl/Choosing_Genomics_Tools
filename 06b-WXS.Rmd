...
```{r, fig.alt = "The learning objectives for this course are to: 1 Define the uses and applications of WXS 2 Describe the steps for generating WXS data 3 Understand the data analysis workflow for WXS", out.width = "100%", echo = FALSE}
ottrpal::include_slide("https://docs.google.com/presentation/d/1YwxXy2rnUgbx_7B7ENH9wpDX-j6JpJz6lGVzOkjo0qY/edit#slide=id.g1b625723c80_0_0")
```
The learning objectives for this course are to explain the use and application of Whole Exome Sequencing (WGS) for genomics studies, outline the technical steps in generating WXS data, and detail the processing steps for analyzing and interpreting WXS data.

## WXS Overview

```{r, fig.alt = "Process of determining coding regions of DNA of an organism’s genome. Includes sequencing anywhere that a open reading frame sequence is bound. Used to identify functional coding sequence variants associated with disease", out.width = "100%", echo = FALSE}
...
If a research question or exploration is largely focused on protein or downstream applications of coding sequences then it makes sense to only sequence these regions to reduce multiple testing burden.

## Advantages and Disadvantages of WGS

```{r, fig.alt = "Advantages:Less expensive to perform than whole genome sequencing. Reduces multiple testing burden by not assaying the coding variants that a research may not be interested in. Allows for discovery to identify causative variants in protein-coding regions. Due to its less expense, can perform deeper sequencing of the regions of interest. Ability to study: Copy number variations, Insertion-Deletions, SNPs, Sequencing repeats, Disadvantages, Non-coding regions are not measured, Still rather expensive, but getting cheaper. ", out.width = "100%", echo = FALSE}
ottrpal::include_slide("https://docs.google.com/presentation/d/1YwxXy2rnUgbx_7B7ENH9wpDX-j6JpJz6lGVzOkjo0qY/edit#slide=id.g1b625723c80_0_12")
```
WXS can be an appropriate assay for many research goals. WXS is preferable to WGS if the research goal does not pertain to noncoding regions. Because of wider assessment of the assay, whole genome sequencing is not only more expensive, but for statistics purposes, reduces a researcher's capability of identifying a protein coding variant of interest because of a principle known as [multiple testing problem](https://en.wikipedia.org/wiki/Multiple_comparisons_problem). Because of this and reasons of cost, if a research question does not pertain to non-coding regions its advisable to use WXS instead of WGS. The lower cost of WXS will also allow the budget for sequencing the regions assayed at a deeper level which can increase the confidence and accuracy of the calls made.  

## Comparing WXS to other modalities
...
```
Compared to WES and Targeted Gene Sequencing, WGS is the most expensive but requires the lowest depth of coverage to achieve 95% sensitivity. In other words, WGS requires sequencing each region of the genome (3.2 billion bases) 30 times in order to confidently be able to pick up all possible meaningful variants. [This article](https://www.nature.com/articles/nrg3642) goes into more depth on how these depths are calculated.
...
WGS: Bentley, D. R. et al. Accurate whole human genome sequencing using reversible terminator chemistry. Nature 456, 53–59 (2008).
WES: Clark, M. J. et al. Performance comparison of exome DNA sequencing technologies. Nature Biotech. 29, 908–914 (2011).
Targeted: Bewicke-Copley F et al. Applications and analysis of targeted genomic sequencing in cancer studies. Comput Struct Biotechnol J. 2019;17:1348-59.
